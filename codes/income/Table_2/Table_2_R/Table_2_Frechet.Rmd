...
Q_train <- array(0, dim=c(dim(Y)[1], (n_levs-1), n_fold))
for (id_fold in 1:n_fold){
  ...
  Q_train[loc_CV != id_fold, , id_fold] <- res_train$qout[, 2:n_levs]
}
...
write.csv(Y_pred_val[, 2:100], file = '../../predictions/qt_test_Frechet.csv')
#write.csv(Y_pred_train[, 2:100], file = '../output/quantiles/qt_train_Frechet.csv')
# import numpy
np = import("numpy")
np$save("../../predictions/qt_train_Frechet.npy",r_to_py(Q_train))
```
...
write.csv(qt_array, file = '../../predictions/res_Frechet_qt.csv')
```
