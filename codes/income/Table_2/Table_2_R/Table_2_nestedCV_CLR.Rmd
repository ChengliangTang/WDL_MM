...
```{r}
# load data
X <- as.matrix(read.csv('../../../../data/income/processed/dat_X.csv'))
Y <- as.matrix(read.csv('../../../../data/income/processed/dat_Y.csv'))
loc_CV <- unlist(read.csv('../../../../data/income/processed/dat_CV.csv'))
loc_CV <- loc_CV + 1
```
...
for (id_test in 1:n_fold){
  ...
  pred_mat <- X_test_full %*% B
  dens_pred[loc_CV == id_test, ] <- t(apply(pred_mat, 1, function(x) return(coef2density(loc_fine, splajn.basis, x))))
  train_mat <- X_train_full %*% B
  dens_train[loc_CV != id_test, , id_test] <- t(apply(train_mat, 1, function(x) return(coef2density(loc_fine, splajn.basis, x))))
}
...
write.csv(quantile_val, file = '../../predictions/qt_test_CLR.csv')
...
# import numpy
np = import("numpy")
np$save("../../predictions/qt_train_CLR.npy",r_to_py(quantile_train))
```
